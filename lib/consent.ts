...
/**
 * Creates a fresh consent request for a child and emails the guardian the link.
 * Any earlier unanswered request for the same child is closed first, so only the newest
 * email works.
 */
export async function issueConsentRequest(
  ...
  const link = `${getSiteUrl()}/guardian/consent?token=${token}`;
  ...
  try {
    await sendMail({
      ...
      html:
        ...
        `border-radius:10px;font-weight:700;text-decoration:none">Read the details and decide</a></p>` +
 ...
 */
export async function hasActiveConsentRequest(admin: Admin, profileId: string): Promise<boolean> {
 ...
 * classes or payments). Used when a guardian declines, and for an under-13 who signed in
 * with Google. Returns false — and deletes nothing — if the account has any activity, in
