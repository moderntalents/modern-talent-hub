...
describe("AFTER 0012: attacks fail, legitimate flows keep working", () => {
  ...
  beforeEach(async () => {
    await db.exec(`
      delete from withdrawal_requests; delete from payment_transactions; delete from subscriptions where student_id = '${ID.S7}';
  ...
  describe("legitimate flows (12–26)", () => {
    ...
    test("19. payment completion (service role) still credits the teacher's wallet by 70%", async () => {
      ...
      const r = await attempt("service", "update payment_transactions set status = 'completed' where teacher_id = $1 returning teacher_share::int as share", [ID.T2]);
