...
export async function newPayment(
  db: PGlite,
  o: { student?: string; activity?: string; amount?: number; checkout?: string } = {},
): Promise<PaymentRef> {
  ...
  const pay = await db.query<{ id: string }>(
    `insert into payment_transactions (subscription_id, student_id, teacher_id, amount, provider, status, checkout_request_id)
     values ($1, $2, $3, $4, 'mpesa', 'pending', $5) returning id`,
    [sub.rows[0].id, student, teacher, amount, o.checkout ?? null],
  );
